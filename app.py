...
def debug_dates():
    books = Book.query.order_by(Book.read_date.desc()).limit(20).all()
    rows = "".join(f"<tr><td>{b.title[:40]}</td><td>'{b.read_date}'</td></tr>" for b in books)
...
# ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ REMOVE ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
@app.route("/remove/<int:index>")
def remove(index):
    library = load_library()
    library.pop(index)
...
# ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ UPDATE STATUS ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
@app.route("/status/<int:index>/<status>")
def update_status(index, status):
    library = load_library()
    library[index]["status"] = status
...
def books():
    library = [b.to_dict() for b in Book.query.all()]
    library = sorted(library, key=lambda b: b.get("read_date") or "", reverse=True)
    return render_template("books.html", books=library)
