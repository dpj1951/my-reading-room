...
- Platform: Render (Starter $7/mo Ã¢ÂÂ always-on, no sleep)
- Flask + Gunicorn
...
- templates/home.html Ã¢ÂÂ logged-in dashboard (nav: Books, Authors, Add a Book, Utilities, Stats)
- templates/stats.html Ã¢ÂÂ reading stats page
...
Auto-lookups page counts from Open Library (by ISBN, then title/author) and Google Books (by ISBN, then title/author) â 4 sources tried in order
...
- **Author shelf feature** Ã¢ÂÂ full publication shelf for each author
  - Author name on authors page is now a clickable pill button
  - New route `/author/<name>` renders author_shelf.html
  - Google Books API called CLIENT-SIDE (browser fetch) Ã¢ÂÂ server-side calls blocked by Render IP
  ...
  - Key bug fixed: year sent as int from JS, route called .strip() Ã¢ÂÂ fixed with str() conversion
...
- New templates/stats.html Ã¢ÂÂ dark theme, DM Serif Display, matches app style
...
- pages and read_date are nullable Ã¢ÂÂ stats work with partial data, users self-edit to improve
...
- Code was already wired up Ã¢ÂÂ no app.py changes needed

### Render Plan Confirmed
- Workspace: Hobby (legacy)
- Service instance: Starter ($7/mo) Ã¢ÂÂ always-on, no sleep
...
2. Second new feature (TBD Ã¢ÂÂ discuss next session)
