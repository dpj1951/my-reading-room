...
- Created 3 responsive logo sizes (user resized manually, maintaining aspect ratio):
  - `alcove_logo_sm.png` (147×220) — mobile
  - `alcove_logo_md.png` (206×307) — tablet/iPad
  - `alcove_logo_lg.png` (270×402) — desktop
- All 3 uploaded to `static/icons/` via GitHub web UI (Upload files drag-and-drop)
- `templates/home.html` footer updated to use `<picture>` element with responsive breakpoints:
  - min-width 1024px → `alcove_logo_lg.png`
  - min-width 768px → `alcove_logo_md.png`
  - default → `alcove_logo_sm.png`
- Logo renders at bottom of home page below nav buttons — may reduce size in a future session
...
- Replaced broken emoji bytes in status buttons with safe HTML entities: â Read, â¶ Reading, ð Want to Read, â DNF
...
- Auto-lookups from Open Library (by ISBN, then title/author) and Google Books (by ISBN, then title/author) â 4 sources
...
### Author shelf feature â full publication shelf per author
...
- pages and read_date are nullable â stats work with partial data
...
- Workspace: Hobby (legacy), Service instance: Starter ($7/mo) â always-on, no sleep
...
- Half-star ratings: upgraded from whole stars (1â5) to half-star increments (0.5â5.0); add.html + edit.html: half-star picker UI; books.html + authors.html: star display with half symbol; rating stored as float
...
- If old page serves after deploy: Render Shell â `kill -9 $(pgrep -f gunicorn)` â Render restarts Gunicorn automatically
...
## Next Tasks
- **Next session: Work on PWA launcher icon** (icon-192.png and icon-512.png)
