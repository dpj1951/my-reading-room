...
Click cover → open edit page → set read date + status=Read → save
...
- Added 📖 Reading / 🔖 Want to Read status badge pills under covers on shelf sections
- Badges are teal for Reading, purple for Want to Read

### authors.html — DONE
Added status badge pills to book rows — teal 📖 Reading, purple 🔖 Want to Read.
Only shown for non-read books. Read books show no badge.

### Landing Page — DONE (April 26)
...
### Currently Reading Shelf CSS — DONE (April 26)
...
User pastes a short-lived token in chat → Claude pushes immediately → user revokes at github.com/settings/tokens
