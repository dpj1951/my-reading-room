# CLAUDE.md ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ My Reading Alcove
...
- Platform: Render (Starter $7/mo ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ always-on, no sleep)
- Flask + Gunicorn
...
- MAINTENANCE_MODE=true in Render env vars ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ site closed to public
...
### Key files
- app.py ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ main Flask app (all routes)
- templates/landing.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ public landing page
- templates/books.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ library view with shelves
- templates/add.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ add book page
- templates/edit.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ edit book page
- templates/authors.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ authors view
- templates/home.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ logged-in dashboard (nav: Books, Authors, Add a Book, Utilities, Stats)
- templates/stats.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ reading stats page

## Book Status System
- 'read' ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ main grid, sorted by read_date desc
- 'reading' ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Currently Reading horizontal scroll shelf (top of books page)
- 'want_to_read' ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Want to Read shelf (bottom of books page)
- Default is 'read' if no status set

## Supabase Schema (books table ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ 14 columns)
...
- pages stored as varchar string ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ parse with int()
...
## Scalability Notes
- Render Starter ($7/mo): always-on, 512MB RAM, 0.5 CPU ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ good for early growth
...
- CSV import does synchronous Google Books/Open Library lookups ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ future: use ThreadPoolExecutor
- Render pricing update coming August 1, 2026 ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ review before that date

## What Was Done May 2, 2026 (continued)
DNF button fix â active highlight and garbled labels
Added .fmt-btn, .fmt-btn:hover, .fmt-btn.active CSS to edit.html and add.html
Replaced broken emoji bytes in status buttons with safe HTML entities:
  &#10003; Read, &#9654; Reading, &#128218; Want to Read, &#10007; DNF
DNF shelf confirmed working â books saved as DNF appear on books page below Want to Read

Missing read dates tool â update
...
Find Missing Read Dates Ã¢ÂÂ updated to show date_added as hint
...
DNF (Did Not Finish) shelf added Ã¢ÂÂ May 2, 2026
...
edit.html: Ã¢ÂÂ DNF button added to Reading Status row
add.html: Ã¢ÂÂ DNF button added to Reading Status row
...
Auto-lookups page counts from Open Library (by ISBN, then title/author) and Google Books (by ISBN, then title/author) ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ 4 sources tried in order
...
- **Author shelf feature** ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ full publication shelf for each author
  - Author name on authors page is now a clickable pill button
  - New route `/author/<name>` renders author_shelf.html
  - Google Books API called CLIENT-SIDE (browser fetch) ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ server-side calls blocked by Render IP
  ...
  - Key bug fixed: year sent as int from JS, route called .strip() ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ fixed with str() conversion
...
- New templates/stats.html ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ dark theme, DM Serif Display, matches app style
...
- pages and read_date are nullable ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ stats work with partial data, users self-edit to improve
...
- Code was already wired up ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ no app.py changes needed

### Render Plan Confirmed
- Workspace: Hobby (legacy)
- Service instance: Starter ($7/mo) ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ always-on, no sleep
...
2. Second new feature (TBD ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ discuss next session)
3. Turn off maintenance mode when ready to launch
4. Review Render pricing changes before August 1, 2026
Shelf label and format button encoding fixed — books.html, edit.html, add.html
Fixed garbled emoji on shelf labels in books.html: &#9654; Reading, &#128218; Want to Read, &#10007; DNF
Fixed garbled emoji on Format buttons in edit.html and add.html: &#128214; Paper, &#128241; Ebook, &#127911; Audiobook
All garbled text fully resolved across books.html, edit.html, add.html
